明白了！你的语音模块是**并行输出**方式——触发后直接在对应的物理引脚上输出高电平，而不是通过串口发送ID数字。你需要树莓派监控8个GPIO引脚，哪个引脚变高就知道执行哪个指令。

根据你图中的引脚对应关系，我重新设计代码：

## 引脚接线方案

| 语音模块引脚 | 功能 | 树莓派GPIO | 物理Pin |
|-----------|------|-----------|--------|
| PA_0 | 唤醒词触发 | GPIO18 | Pin 12 |
| PA_1 | 前进触发 | GPIO17 | Pin 11 |
| PA_4 | 停止触发 | GPIO22 | Pin 15 |
| PC_4 | 后退触发 | GPIO23 | Pin 16 |
| PB_5 | 左转触发 | GPIO24 | Pin 18 |
| PB_6 | 右转触发 | GPIO25 | Pin 22 |
| PA_2 | 摄像头开 | GPIO27 | Pin 13 |
| (新增) | 摄像头关 | GPIO19 | Pin 35 |

> **注意**：你图中只显示了7个动作引脚，缺少"关闭摄像头"的对应引脚。如果语音模块确实有8号指令的物理引脚，请补充告诉我；如果模块只有7个输出，我可以在代码中用其他方式处理关闭摄像头。
...
### 2. **边沿中断检测（推荐）**
- **GPIO.RISING**：检测低电平→高电平的跳变，精确捕获触发时刻
- **bouncetime=50**：硬件防抖，避免机械触点抖动导致多次触发
- **软件防抖300ms**：双重保险，确保一个语音指令只执行一次

### 3. **独立防抖计时**
```python
self._last_trigger_time: Dict[int, int] = {}  # 每个引脚独立计时
```
每个引脚有自己的上次触发时间，避免不同指令之间互相干扰。

## 接线确认

请核对你的语音模块实际引脚：

| 你的模块引脚 | 树莓派GPIO | 我代码中的变量名 |
|-----------|-----------|--------------|
| PA_0 | GPIO18 | `VOICE_WAKEUP` |
| PA_1 | GPIO17 | `VOICE_FORWARD` |
| PA_4 | GPIO22 | `VOICE_STOP` |
| PC_4 | GPIO23 | `VOICE_BACKWARD` |
| PB_5 | GPIO24 | `VOICE_LEFT` |
| PB_6 | GPIO25 | `VOICE_RIGHT` |
| PA_2 | GPIO27 | `VOICE_CAM_ON` |
| ? | ? | `VOICE_CAM_OFF` |

**问题**：图中只显示了7个动作（PA_0到PA_2、PA_4、PB_5、PB_6、PC_4），缺少"关闭摄像头"（原ID 2）对应的物理引脚。请确认：
1. 语音模块是否真的有8个物理输出引脚？
2. 如果有，第8个引脚名称是什么（如PB_x、PC_x）？
